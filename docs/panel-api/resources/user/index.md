...
* `value` - JSON object. [User object](index.md#user-object-structure) described above.
...
Updates existing user with new field values (see [user object](index.md#user-object-structure)). User must exist and must belong to authorized dealer. Changing of `legal_type` is not permitted, i.e. this field will not be changed.
...
Gets list of user's billing transactions for the specified period. Same as [/transaction/list](../../../user-api/resources/commons/billing/transaction.md#list) from main api.
