...
To create a geofence, use the [zone/create](broken-reference) API call. There are several types of geofences, and the creation process varies between them.
...
To get the geofence name or ID where a device is located, use the [zone/search\_location](broken-reference) API call. For example, to determine which geofence a device is in or to count how many devices are in a specific zone, first request the device's [state and location](broken-reference). With the received latitude and longitude parameters, you can then check the geofences.
