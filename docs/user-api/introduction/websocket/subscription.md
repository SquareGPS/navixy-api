...
> * `events` – required, [enum](../#data-types) array, without nulls. List of events to subscribe. An event can only be a `state`.
...
* `format` – optional, [enum](../#data-types), one of: "full" (default), "compact".
...
* `events` – required, array of [enum](../#data-types), without nulls. List of the subscribed events types ("", "" or "iot\_monitor").
...
Error codes are similar to the [API errors codes](../errors.md#error-codes).
...
* `status` – required – error code and description:
  * `code` – required – error code (see [API errors codes](../errors.md#error-codes)).
