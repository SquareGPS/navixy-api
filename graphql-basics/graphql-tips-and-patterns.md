...
GraphQL APIs are self-described. You can query the schema itself using special fields that start with `__` (double underscore).

#### Exploring a type

To see all fields available on a type:

```graphql
query IntrospectDeviceType {
  __type(name: "Device") {
    name
    description
    fields {
...
This query returns the entire schema — all types, fields, enums, and their descriptions:
...
GraphQL tools like GraphiQL and Apollo Sandbox run this query automatically to provide autocomplete and documentation.

{% hint style="warning" %}
Instead of writing introspection queries manually, use the [GraphQL Sandbox](https://sandbox.navixy.com) to explore the schema interactively.
{% endhint %}
